...
class QuartznetModel(DispatchModel):
    ...
    def __init__(self, model_path,
                 ...
                 cutoff_top_n: int = 40,  # 总的字符大于多少截至
        ...
        if not torch.cuda.is_available():
            logging.warning("无法获取到gpu资源，语音识别使用cpu推理，速度会变慢...")
            device = "cpu"
        self.device = torch.device(device)  # 设备
        logging.debug("asr_model初始化前总内存占用 %.4f" % (psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024 / 1024))
        self.asr_model = nemo_asr.models.EncDecCTCModel.restore_from(
            restore_path=model_path, map_location=self.device)
        logging.debug("asr_model初始化后总内存占用 %.4f" % (psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024 / 1024))
        ...
        logging.debug(
            "asr_model to cuda后总内存占用 %.4f" % (psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024 / 1024))
        self.preprocessor = nemo_asr.modules.AudioToMelSpectrogramPreprocessor.from_config_dict(cfg.preprocessor)
        logging.debug("preprocessor后总内存占用 %.4f" % (psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024 / 1024))
        ...
        if use_lm:
            ...
            self.beam_search_lm = None
        self.asr_model.eval()
        logging.debug("总内存占用 %.4f" % (psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024 / 1024))

    def schedulable_infer(self, input_lists: list):
        if self.use_lm:
            logging.info("使用语言模型推理")
            return self.infer_wavs_withlm(input_lists)[0]
    ...
    def infer_signal(self, signal: np.ndarray = None):
        ...
        encoded, encoded_len = self.asr_model.encoder(audio_signal=processed_signal.to(self.device),
                                                      length=processed_signal_length.to(self.device))
    ...
    def infer_wavs_withlm(self, wav_ios: Union[List[str], List[io.BytesIO]] = None):
        ...
        greedy_texts, log_probs, encoded_len, total_secs = self.infer_wavs(wav_ios=wav_ios)
        ...
        lm_texts = [item[0][1] for item in lm_texts]
        logging.info("无语言模型文本: " + str(greedy_texts))
        logging.info("语言模型文本: " + str(lm_texts))
        return lm_texts, log_probs, encoded_len, total_secs
    ...
    @torch.no_grad()
    def infer_wavs(self, wav_ios: Union[List[str], List[io.BytesIO]] = None) -> Any:
        """
        将多个音频打包为一个batch
        ...
        texts = self.wer.ctc_decoder_predictions_tensor(
            greedy_predictions, predictions_len=encoded_len, return_hypotheses=False)
        logging.info(texts)
        return texts, log_probs, encoded_len, total_secs
